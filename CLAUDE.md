...
Numbered top-level folders. The three that matter most:
...
- **`01 - Website/room-parent-portal-redesign/`** — the Room Parent Portal
  prototype. Also **its own nested git repo** (`Fauz2808/roomparent-portal`,
...
- **2026-09-23 — Sprout Assist order: reminders and events first, get them right, then birthdays,
  ...
  "I just don't wanna hit two buttons"; background noise makes a transcript junk, and the AI sorts
  it out after. "Reminders might be a pretty powerful thing for us." *How to apply:* canonical
  prototype `08 …/Home/daily-brief-reminder-loop.html`, Figma copy `12798:18362`. Portal handoffs
  that span several releases go out as ONE cumulative issue
  (`handoff.mjs --since <Mohit's last built version>`), not N release links.
- **2026-09-15 — Engineering handoff runs on versioned releases, not messages.**
  The portal prototype is its own private repo (`Fauz2808/roomparent-portal`);
  each handoff is a GitHub Release whose notes are generated by
  `node tools/handoff.mjs`. *Why:* Mohit twice built against a version Ahmad had
  already replaced, and neither side could say which version he held. The fix is
  identity plus delta: every build carries a stamp (sidebar + nav drawer +
  `prototype-version` meta), and the generator diffs screens and data shapes
  against the previous release so the notes say "button X removed from
  `sec-rptodo`" instead of handing over a 375KB diff. *How to apply:* never send
  a screenshot or a file as a handoff. Run `./tools/release.sh`, send the release
  link. Questions from engineering go in that repo's Issues, not chat.
