...
function setStory(key) {
  ...
  document.querySelector("#scene-number").textContent =
    `0${Object.keys(screens).indexOf(key) + 1} / 03`;
...
setStory(
  screens[location.hash.replace("#story-", "")]
...
const chapterObserver = new IntersectionObserver(
  (entries) => {
    ...
    if (candidates.length)
      setStory(candidates[candidates.length - 1].target.dataset.story);
  },
